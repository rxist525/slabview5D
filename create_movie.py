...
# Try to import cpp_zarr, otherwise mock for dry-run/dev if not present
try:
    import cpp_zarr
except ImportError:
    print("Warning: cpp_zarr module not found. Zarr loading will fail unless mocked.", file=sys.stderr)
    cpp_zarr = None
...
def load_zarr_data(path, mock=False):
    ...
    if cpp_zarr:
        ...
        return cpp_zarr.read_zarr(str(path))
    else:
        raise ImportError("cpp_zarr is required to load data. Use --mock to test without it.")
